...
Kept close to upstream so it can be re-synced; the one behavioural change is
marked DEVIATION below and explained in NOTICE.
...
from .models import Fill, FillResult, OrderBook
...
def calculate_fee(fee_rate_bps: int, price: float, size: float) -> float:
    ...
    size is a share count, not a dollar amount.
...
def simulate_buy_fill(
    ...
    # DEVIATION from upstream: upstream passes total_cost (USD) here while
    # the sell path passes total_shares. The documented formula charges per
    # share, so upstream understated buy fees by a factor of avg_price and
    # disagreed with its own sell path. See NOTICE.
